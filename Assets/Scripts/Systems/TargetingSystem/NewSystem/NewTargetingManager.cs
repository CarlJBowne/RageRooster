...
namespace RageRooster.TargetingV2
{

    public class NewTargetingManager : MonoBehaviour
    {
        // Static runtime state
        public static NewTargetingManager Instance { get; private set; }
        ...
        private void FixedUpdate()
        {
            ...
            if (ShowDebugReticles)
            {
                ...
                {
                    interactionDebugReticle.rectTransform.position = Cameras.RealCamera.camera.WorldToScreenPoint(InteractionChannel.CurrentTarget.This.position);
                }
                if (MeleeChannel.CurrentTarget != null)
                {
                    meleeDebugReticle.rectTransform.position = Cameras.RealCamera.camera.WorldToScreenPoint(MeleeChannel.CurrentTarget.This.position);
                }
                if (RangedChannel.CurrentTarget != null)
                {
                    rangedDebugReticle.rectTransform.position = Cameras.RealCamera.camera.WorldToScreenPoint(RangedChannel.CurrentTarget.This.position);
                }
            }
        }
        ...
        {
            TargetType.Interactable target = InteractionChannel.GetCurrentTarget<TargetType.Interactable>();
            if (target)
            {
        ...
        public static Target GetMeleeTarget() => MeleeChannel.CurrentTarget.This;
    ...
    public class TargetingChannel
    {
        ...
        public T GetCurrentTarget<T>() where T : TargetType => CurrentTarget as T;
        ...
        public void CalculateTargets()
        {
            ...
            if (ChosenTarget != CurrentTarget)
            {
                ...
                if (NewTargetingManager.ShowDebugReticles && debugDisplayReticle != null)
                {
